...
void main() {
        ...
        float screenHeight = iResolution.y * 2.0;
        vec2 tempFragCoord = vec2(gl_FragCoord.x, screenHeight - gl_FragCoord.y);
        tempFragCoord.x = tempFragCoord.x - (resolution.x / 2.0);
        tempFragCoord.y = tempFragCoord.y - (resolution.y / 2.0);
        float time = iTime;
        float xpart = tempFragCoord.x/resolution.x;
        float ypart = tempFragCoord.y/resolution.y;
	float clip = 500.0;
        float ypartClip = tempFragCoord.y/clip;
	...
	//
        //vec2 coordScaled = 0.01*tempFragCoord.xy - 0.02*vec2(offset.x,0.0);
        vec2 coordScaled = 0.01*tempFragCoord.xy;
	...
	//
	float flames = pow(ypartClipped,0.5*xfuel)*pow(noise,0.3*xfuel);
	//
	...
	//
	// smoke
	float smokeNoise = 0.8+snoise(0.5*position+timing*vec3(1.0,1.0,0.2))/2.0;
	vec3 smoke = vec3(0.5*pow(xfuel,3.0)*pow(ypart,2.0)*(smokeNoise+0.5*(1.0-noise)));
	//
	// sparks
	float sparkGridSize = 30.0;
        //vec2 sparkCoord = tempFragCoord.xy - vec2(2.0*offset.x,190.0*realTime);
        vec2 sparkCoord = tempFragCoord.xy;
